...
> 이 문단은 최초 배치 A 캠페인의 역사적 범위를 기록한다. 후속 Fable Directive 12에서 배치 B(M-11·12·14)까지 구현·검증 대상으로 승격했으며, 현재 완료 범위와 남은 배치는 `P13_REVISION_DESIGN_SYNC.md` §6.4를 따른다.

최초 캠페인에 구현한 몬스터 슬라이스는 **배치 A(M-01·02·03·04·07·13)**였다. 후속 Directive 12는 배치 A 골든 안정과 상태이상 UI 진입 조건을 통과해 배치 B(M-11·12·14 — 중독·치유 차단·턴 종료 응징·나이테 원자)를 추가했다. 후속 Directive 13은 다중 적 대상 우선순위 UX 플레이테스트 진입 조건을 확인한 뒤 배치 C(M-05·06·08 — 보호 재지정·감쇄 임계·오라·같은 해결의 사망 정리)를 추가했다. D(M-09·10·17·18 — 압수·봉인·위조·속성세), E(M-15·16 — 적측 소환), F(M-19·20 — 보스 페이즈 복합)는 대기이며, 배치 E의 사망 정리 선행 조건만 충족했다. 각 배치의 현재 진입 조건은 P13 설계 정본 §6.4 표를 따른다.
...
- 배치 C는 최초 Wave 5 소유권 표를 소급 변경하지 않는다. 이 계획의 W5a~W5d는 배치 A 최초 구현의 역사 기록이다.
- Directive 13의 실제 범위는 공유 피해 해결기, 선택적 `EnemyDef` 배치 C 필드, 적 사망 정리, 배치 C 콘텐츠·2·3막 풀, 의도/상태 UI, 계약 테스트다. 최종 게이트 수치와 번들 측정은 [`P13_VERIFICATION_EVIDENCE.md`](./P13_VERIFICATION_EVIDENCE.md)의 Directive 13 후속 기록에서 실측 후 채운다.
