...
const base =
  ...
  "http://127.0.0.1:4183/deckbuilding-roguelite/";
...
await combatPage.getByTestId("combat-preferences-open").click();
...
await combatPage.getByTestId("preference-high-contrast").check();
report.push(
  ...(await measure(combatPage, [
    [".unit-name", "고대비 유닛 이름"],
    [".card-title", "고대비 카드 제목"],
    [".end-turn", "고대비 턴 종료 버튼"],
  ])),
);
...
await shopPage.waitForSelector('[data-testid="shop-screen"]', {
  timeout: 15000,
});
report.push(
