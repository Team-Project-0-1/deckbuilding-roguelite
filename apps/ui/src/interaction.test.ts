...
describe("cardActionView — 소비형 카드 하단 행동 바", () => {
  ...
  it("대상 선택·해결 중·쿨다운·전투 잠금의 우선순위를 보존한다", () => {
    expect(cardActionView({ ...base, targeting: true })).toMatchObject({
      actionable: true,
      label: "대상 선택 중 · 취소",
      tone: "targeting",
    });
    expect(
      cardActionView({ ...base, resolving: true, targeting: true }),
    ).toMatchObject({
      actionable: false,
      label: "발동 중…",
      tone: "busy",
    });
    expect(cardActionView({ ...base, cooldownRemaining: 2 })).toMatchObject({
      actionable: false,
      label: "재사용까지 2턴",
      tone: "idle",
    });
    expect(cardActionView({ ...base, usedThisCombat: true })).toMatchObject({
      actionable: false,
      label: "이번 전투 사용 완료",
      tone: "idle",
    });
  ...
  it("소비 선택은 선택량과 확정 가능 여부를 같은 바에 표시한다", () => {
    expect(
      cardActionView({
        ...base,
        loaded: 1,
    ...
    ).toEqual({ actionable: false, label: "1/2 소비", tone: "idle" });
    expect(
      cardActionView({
        ...base,
        loaded: 2,
    ...
    ).toEqual({ actionable: true, label: "2/2 소비 · 확정", tone: "ready" });
  });
});
  ...
  it("groups identical permanent coins separately when their immutable enchants differ", () => {
    ...
    const synthetic = {
      ...
      coins: {
        1: {
          uid: 1 as CoinUid,
          permanentUid: 1 as never,
          defId: "basic" as never,
          permanent: true,
          grants: [],
          enchant: "sharpness" as never,
        },
        2: {
          uid: 2 as CoinUid,
          permanentUid: 2 as never,
          defId: "basic" as never,
          permanent: true,
          grants: [],
          enchant: "heads-polish" as never,
        },
        3: {
          uid: 3 as CoinUid,
          permanentUid: 3 as never,
          defId: "basic" as never,
          permanent: true,
          grants: [],
          enchant: "sharpness" as never,
      ...
      zones: {
        ...base.zones,
        draw: [1 as CoinUid, 2 as CoinUid, 3 as CoinUid],
      },
    } as CombatState;
...
describe("pendingLoadedCoinCount — 미발동 장전 턴 종료 가드", () => {
  ...
  it("스킬 해결 뒤 비워진 슬롯은 세지 않는다", () => {
    const loaded = placeFirst(boot(), 0).state;
    const used = step(
      loaded,
    ...
    if (!used.ok) return;
    expect(pendingLoadedCoinCount(used.state)).toBe(0);
  });
});
...
describe("drawPileComposition — 뽑을 더미 구성 (종류·매수만, 순서 비공개)", () => {
  it("그룹 합계가 뽑을 더미 매수와 일치하고 종류별 매수가 실제와 같다", () => {
    const state = boot(); // 주머니 10 + 불씨 임시 화염 1 - 드로우 3 = 8
    const groups = drawPileComposition(state, contentDb);
    expect(groups.reduce((sum, group) => sum + group.count, 0)).toBe(8);
    expect(state.zones.draw.length).toBe(8);
  ...
  it("합성 상태: 기본(영구·임시)과 화염을 분리 집계하고 기본→화염, 영구→임시 순으로 정렬한다", () => {
    ...
    expect(drawPileComposition(synthetic, contentDb)).toEqual([
      { defId: "basic", element: null, grants: [], temporary: false, enchant: null, count: 2 },
      { defId: "basic", element: null, grants: [], temporary: true, enchant: null, count: 1 },
      {
        defId: "fire",
        element: "fire",
        grants: [],
  ...
  it("턴 전환 후 구성이 갱신된다 — 2턴 드로우 뒤 더미 5닢", () => {
    const state = boot();
    const ended = step(state, { type: "endTurn" }, contentDb);
    if (!ended.ok) throw new Error(ended.error);
    const groups = drawPileComposition(ended.state, contentDb);
    expect(ended.state.zones.draw.length).toBe(5);
    expect(groups.reduce((sum, group) => sum + group.count, 0)).toBe(5);
  });
});
...
describe("pileComposition — 버림·소모 영역 구성과 수명주기 표식", () => {
  it("버림과 소모를 분리 집계하고 취급 속성·영구/임시를 구분한다", () => {
    ...
    expect(pileComposition(synthetic, "discard", contentDb)).toEqual([
      { defId: "basic", element: null, grants: [], temporary: false, enchant: null, count: 1 },
      {
        defId: "basic",
        element: null,
        grants: ["fire"],
    ...
    expect(pileComposition(synthetic, "exhausted", contentDb)).toEqual([
      ...
      { defId: "fire", element: "fire", grants: [], temporary: true, enchant: null, count: 1 },
    ]);
  });
});
...
describe("dropCommands / stepSequence — 드롭의 커맨드 변환과 전량 커밋", () => {
  ...
  it("서로 다른 카드에 장전된 동전을 드래그해 맞바꾼다", () => {
    ...
    const commands = dropCommands(
      first.coin,
      { kind: "socket", slot: slot(0) },
      { kind: "slot", slot: slot(1), coin: second.coin },
    );

    expect(commands).toEqual([
      { type: "unplaceCoin", coin: first.coin },
      { type: "unplaceCoin", coin: second.coin },
      { type: "placeCoin", coin: second.coin, slot: slot(0) },
  ...
  it("같은 카드의 두 장전 소켓도 순서를 맞바꾼다", () => {
    ...
    const commands = dropCommands(
      first.coin,
      { kind: "socket", slot: slot(2) },
      { kind: "slot", slot: slot(2), coin: second.coin },
    );
    const run = stepSequence(second.state, commands ?? [], contentDb);
  ...
  it("양쪽 스킬 제한을 모두 만족하는 동전만 교환 대상으로 표시한다", () => {
    const first = placeFirst(boot(), 0);
    const second = placeFirst(first.state, 1);
    ...
    ).toEqual(new Set([Number(second.coin)]));
  });
});
