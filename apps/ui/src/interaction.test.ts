...
import type { CoinUid, CombatEvent, CombatState, SlotId } from '@game/core';
...
const boot = (): CombatState =>
  createCombat({ character: 'warrior' as never, enemies: ['raider' as never] }, contentDb, 'interaction-test');

const placeFirst = (state: CombatState, slotIndex: number): { state: CombatState; coin: CoinUid } => {
  const coin = state.zones.hand[0];
  if (coin === undefined) throw new Error('missing hand coin');
  const result = step(state, { type: 'placeCoin', coin, slot: slot(slotIndex) }, contentDb);
...
describe('coinFacesAfterEvent — 면 기록 수명주기', () => {
  it('coinFlipped가 면을 기록한다', () => {
    const faces = coinFacesAfterEvent({}, { type: 'coinFlipped', coin: 3 as CoinUid, face: 'heads' });
    expect(faces[3]).toBe('heads');
  });

  it('같은 코인의 재플립이 면을 덮어쓴다', () => {
    const first = coinFacesAfterEvent({}, { type: 'coinFlipped', coin: 3 as CoinUid, face: 'heads' });
    const second = coinFacesAfterEvent(first, { type: 'coinFlipped', coin: 3 as CoinUid, face: 'tails' });
  ...
  it('다시 뽑힌 코인의 낡은 면은 지워진다 — 뽑힌 동전은 아직 안 굴린 동전이다', () => {
    const flipped = coinFacesAfterEvent({}, { type: 'coinFlipped', coin: 3 as CoinUid, face: 'heads' });
    const drawn: CombatEvent = { type: 'coinsDrawn', coins: [3 as CoinUid, 4 as CoinUid] };
  ...
  it('무관한 코인 드로우는 기존 면을 보존한다', () => {
    const flipped = coinFacesAfterEvent({}, { type: 'coinFlipped', coin: 3 as CoinUid, face: 'tails' });
    const next = coinFacesAfterEvent(flipped, { type: 'coinsDrawn', coins: [7 as CoinUid] });
  ...
  it('그룹 합계가 뽑을 더미 매수와 일치하고 종류별 매수가 실제와 같다', () => {
    ...
    for (const group of groups) {
      const actual = state.zones.draw.filter((coin) => {
        ...
        return String(instance?.defId) === group.defId && instance?.permanent === !group.temporary;
      }).length;
  ...
  it('합성 상태: 기본(영구·임시)과 화염을 분리 집계하고 기본→화염, 영구→임시 순으로 정렬한다', () => {
    ...
    const synthetic: CombatState = {
      ...
      coins: {
        ...
        2: { uid: 2 as CoinUid, defId: 'basic' as never, permanent: false, grants: [] },
        3: { uid: 3 as CoinUid, defId: 'fire' as never, permanent: true, grants: [] },
        4: { uid: 4 as CoinUid, defId: 'basic' as never, permanent: true, grants: [] }
      },
...
describe('dragTargetSlots — 드래그 합법 목적지 (코어 판정 위임)', () => {
  it('손패 코인은 코스트가 남은 flip 슬롯 전부가 목적지다', () => {
    ...
    const coin = state.zones.hand[0];
    if (coin === undefined) throw new Error('missing hand coin');
    // 전사 슬롯: 0 베기 / 1 방어 / 2 불타는 일격 / 3 점화 / 4 점화 검술(소비) / 5 화염 폭주
    expect([...dragTargetSlots(state, coin, { kind: 'hand' }, contentDb)].sort()).toEqual([0, 1, 2, 3, 5]);
  });
  ...
  it('가득 찬 슬롯은 목적지에서 빠진다', () => {
    ...
    const targets = dragTargetSlots(state, another, { kind: 'hand' }, contentDb);
  ...
  it('소켓 출발은 회수 후 장전이 둘 다 합법인 슬롯만 — 자기 슬롯 제외', () => {
    ...
    const targets = dragTargetSlots(state, coin, { kind: 'socket', slot: slot(0) }, contentDb);
    expect(targets.has(0)).toBe(false);
    expect([...targets].sort()).toEqual([1, 2, 3, 5]);
  });
});
...
describe('dropCommands / stepSequence — 드롭의 커맨드 변환과 전량 커밋', () => {
  it('손패 → 합법 소켓 = placeCoin 1건', () => {
    ...
    const coin = state.zones.hand[0];
    if (coin === undefined) throw new Error('missing hand coin');
    const commands = dropCommands(coin, { kind: 'hand' }, { kind: 'slot', slot: slot(2) });
  ...
  it('소켓 → 트레이 = 회수, 소켓 → 다른 슬롯 = 회수+장전 이동', () => {
    const { state, coin } = placeFirst(boot(), 0);
    const toTray = dropCommands(coin, { kind: 'socket', slot: slot(0) }, { kind: 'tray' });
    expect(toTray).toEqual([{ type: 'unplaceCoin', coin }]);

    const move = dropCommands(coin, { kind: 'socket', slot: slot(0) }, { kind: 'slot', slot: slot(1) });
    expect(move).toEqual([
  ...
  it('열 중 하나라도 불법이면 전체 취소 (null)', () => {
    const { state, coin } = placeFirst(boot(), 0);
    // 소비형 슬롯(4)으로의 이동은 placeCoin이 불법 → 시퀀스 전체가 null
    const bad = dropCommands(coin, { kind: 'socket', slot: slot(0) }, { kind: 'slot', slot: slot(4) });
