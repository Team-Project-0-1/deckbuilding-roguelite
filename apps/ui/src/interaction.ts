...
export const coinFacesAfterEvent = (faces: CoinFaces, event: CombatEvent): CoinFaces => {
...
export const dragTargetSlots = (state: CombatState, coin: CoinUid, source: DragSource, db: ContentDb): Set<number> => {
  let base = state;
  ...
  for (const command of legalCommands(base, db)) {
    if (command.type === 'placeCoin' && command.coin === coin) targets.add(Number(command.slot));
  }
...
export const stepSequence = (
  ...
  for (const command of commands) {
    const legal = legalCommands(current, db).some((candidate) => sameCommand(candidate, command));
