...
- 코스트 1 성공 단계형도 0개 성공 단계에 효과를 둘 수 있다. 이는 실패 저점일 뿐 성공으로 세지 않으며 공명이나 성공면 전용 프로크를 발동하지 않는다. 빈 0단계도 계속 유효하다.
...
쿨다운 0 플립 스킬은 기본적으로 반복 예약 가능하다. 서로 다른 동전으로 완전한 예약을 만들 수 있는 동안 같은 턴에 여러 번 예약하며, 각 예약은 별도 플립·대상·실행 순서를 가진다. 기본/양수 쿨다운, `oncePerCombat`, `nonRepeatable` 스킬은 첫 예약 뒤 잠긴다. 부분 장전은 예약이 아니며 실행 큐에 들어가지 않는다.
...
- P13 배치 A 기믹 필드(전부 옵셔널, 계약은 `packages/core/src/combat/enemy-atoms.test.ts`): `windup {turns, cancelOn}`. `cancelOn`은 스킬 피해 취소 `{ kind: 'skillDamage', threshold }`와 용광로 자원 취소 `{ kind: 'enemyResourceAtMost', resource: 'furnaceTemperature', value }`의 판별된 형태를 쓴다. 그 밖에 `vulnerableWhileWindup`, `conditionalAttack(playerHpBelowHalf)`, `phases {hpBelowFraction, damageTakenMultiplier, intents}`, `attack.damagePerGrowthPercent`, `growOnUnblockedDamage(+healOnGrow/maxStacks/minHpDamageFraction/loseOnFullBlock)`, `growthBranch {atLeast, intent}`, `healAlly(lowestHpAlly, cleanse)`를 사용한다.
