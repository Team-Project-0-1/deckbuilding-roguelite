...
export const RUN_SAVE_VERSION = 10 as const;
export const LEGACY_RUN_SAVE_VERSIONS = [1, 2, 3, 4, 5, 6, 7, 8, 9] as const;
...
export interface PendingRewards {
  ...
  /** Present only for elite/boss enchanted-coin offers; index-aligned to coinOptions. */
  coinEnchantOptions?: CoinEnchantId[];
...
export interface PermanentCoinRecord {
  uid: PermanentCoinUid;
  defId: CoinDefId;
  readonly enchant?: CoinEnchantId;
}

export interface PermanentCoinLedger {
  nextUid: number;
  coins: PermanentCoinRecord[];
}
...
export interface RunSave {
  ...
  maxHp: number;
  bag: CoinDefId[];
  /** Per-copy permanent identity. Kept index-aligned with bag for v9 API compatibility. */
  permanentCoins: PermanentCoinLedger;
